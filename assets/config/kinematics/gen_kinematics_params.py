...
if __name__ == "__main__":
    ...
    if len(recv_data) == 179 and recv_data[8]:
        ...
        robot_name = (
            ...
            else "xarm{}".format(robot_dof)
        )
        ouput_dir = os.path.join(os.path.dirname(__file__), "user")
        if not os.path.exists(ouput_dir):
            os.makedirs(ouput_dir)
        output_file = os.path.join(
            ouput_dir, "{}_kinematics_{}.yaml".format(robot_name, kinematics_suffix)
        )
        ...
        print(
            "[Failed] recv_len={}, valid={}".format(
                len(recv_data), 0 if len(recv_data) < 9 else recv_data[8]
            )
        )
